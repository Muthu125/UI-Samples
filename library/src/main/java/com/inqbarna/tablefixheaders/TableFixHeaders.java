    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: {
				...
				/*	if(mOnScrolListener != null) {
						MSFLog.msg(this.getClass().getSimpleName(), "mOnScrolListener finished onTouchEvent ACTION_DOWN");
						mOnScrolListener.onScrollFinished();
					}*/
                }
                currentX = (int) event.getRawX();
                currentY = (int) event.getRawY();
...
//			scrollBy(diffX, diffY);
                ...
                if (mOnScrolListener != null) {
                    if (diffY > 10) {
                        mOnScrolListener.onVerticalScrollUp();
                    } else if (diffY < -10) {
                        mOnScrolListener.onVerticalScrollDown();
                    }
                    if (diffX < -10) {
                        mOnScrolListener.onHorizontalScrollLeft();
                    } else if (diffX > 10) {
                        mOnScrolListener.onHorizontalScrollRight();
                    }
                }
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (Math.abs(velocityX) > minimumVelocity
                    ...
                    if(isscrollingx) {
                        flinger.start(getActualScrollX() , 0,
                                velocityX, 0, getMaxScrollX() +100 , 0);
                    }else {
                        if(widths != null) {
                            flinger.start(0, getActualScrollY(),
                                    0, velocityY, 0, getMaxScrollY() + 100);
                        }
                    }
                } else {
                    Log.d("Event: " +" Tracker is less","");
                    if (this.velocityTracker != null) { // If the velocity less than
                        // threshold
                        this.velocityTracker.recycle(); // recycle the tracker
                        this.velocityTracker = null;
                    }
                    if(mOnScrolListener != null && !isscrollingx) {
                        Log.d(this.getClass().getSimpleName(), "mOnScrolListener finished onTouchEvent ACTION_UP");
                        mOnScrolListener.onScrollFinished(firstRow, (firstRow +  columnViewList.size() - 1) );
                    }
                }
                break;
            }
    ...
    public interface onScrollListener {
        ...
        void onVerticalScrollUp();

        void onVerticalScrollDown();

        void onHorizontalScrollLeft();

        void onHorizontalScrollRight();

        /**
         * The scroll from row to column
         * @param  firstVisiblerow - first visible item.
         * @param lastVisibleRow - last visible item
         */
        void onScrollFinished(int firstVisiblerow, int lastVisibleRow);

        void onTopReached();

        void onHorizontalSwipeCompleted();

        void onHorizontalSwipeStarted();
    }

    public onScrollListener mOnScrolListener;

    public void setonScrollListener(onScrollListener mOnScrollListeners) {
        mOnScrolListener = mOnScrollListeners;
    }
    ...
    public void scrollBy(int x, int y) {
        ...
        if (scrollX == 0) {
            ...
            if(mOnScrolListener != null && firstRow == 0) {
                mOnScrolListener.onHorizontalSwipeCompleted();
            }
            // no op
        } else if (scrollX > 0) {

            if(mOnScrolListener != null ) {
                mOnScrolListener.onHorizontalSwipeStarted();
            }
        ...
        Log.d("TableFixHeaders scrolly zero " + firstRow + columnViewList.size(),"");
        repositionViews();
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (row == null || (row == -1 && column == -1)) {
            ...
            canvas.save();
            if (row == -1) {
                canvas.clipRect(widths[0], 0, canvas.getWidth(),
                        canvas.getHeight());
            } else if (column == -1) {
    ...
    private class Flinger implements Runnable {
        ...
        void forceFinished() {
            if (!scroller.isFinished()) {
                ...
                if(mOnScrolListener != null && !isscrollingx) {
                    Log.d(this.getClass().getSimpleName(), "mOnScrolListener finished forceFinished scroller");
                    mOnScrolListener.onScrollFinished(firstRow, (firstRow +  columnViewList.size() - 1) );
                }
            }
        }
    ...
    public int getNumberOfRowsVisible () {
        if(columnViewList != null ) {
            return  columnViewList.size() - 1;
        }
        return 0;
    }
}
