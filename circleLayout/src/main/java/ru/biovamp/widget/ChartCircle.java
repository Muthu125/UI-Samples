...
public class ChartCircle extends ViewGroup {
	...
	public ChartCircle(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.ChartCircle, 0, 0);

		try {
			int dividerColor = a.getColor(R.styleable.ChartCircle_sliceDivider, android.R.color.darker_gray);
			mInnerCircle = a.getDrawable(R.styleable.ChartCircle_innerCircle);

			if(mInnerCircle instanceof ColorDrawable) {
				int innerColor = a.getColor(R.styleable.ChartCircle_innerCircle, android.R.color.white);
				mCirclePaint.setColor(innerColor);
			}

			mDividerPaint.setColor(dividerColor);

			mAngleOffset = a.getFloat(R.styleable.ChartCircle_angleOffset, 90f);
			mAngleRange = a.getFloat(R.styleable.ChartCircle_angleRange, 360f);
			mDividerWidth = a.getDimensionPixelSize(R.styleable.ChartCircle_dividerWidth, 1);
			mInnerRadius = a.getDimensionPixelSize(R.styleable.ChartCircle_innerRadius, 80);

			mLayoutMode = a.getColor(R.styleable.ChartCircle_layoutMode, LAYOUT_NORMAL);
		} finally {
			a.recycle();
		}
